...
int dfs( vector<bool>&vis,unordered_map<int, vector<int> >&graph, vector<int>& cgpa,bool & flag ,int v , int k , int prevmin){
    ...
    for(auto i: graph[v]){
        if(vis[i]==0){
            vis[i]=1;
            prevmin= min( prevmin, dfs(vis,graph, cgpa, flag,i, k-1, prevmin) );
        }
    }
...
void solve(){
    ...
    for(int i=0; i<n; i++){
        ...
        neighbour=dfs(vis,graph, cgpa, foundt, i, k-1, tempres);
